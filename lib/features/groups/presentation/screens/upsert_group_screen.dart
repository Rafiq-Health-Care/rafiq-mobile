...
class _UpsertGroupScreenState extends State<UpsertGroupScreen> {
  ...
  final ValueNotifier<Color> _selectedColor = ValueNotifier(Colors.blue);
  ...
  void initState() {
    ...
    if (widget.group != null) {
      _selectedColor.value = Colors.amber;
    }
  }
  ...
  void _onSave() {
    if (_formKey.currentState!.validate()) {
      final request = GroupUpsertRequest(
        ...
        color: widget.group != null ? "RED" : "BLUE",
      );

      if (widget.group == null) {
        GroupCubit.of(context).addGroup(request);
      } else {
        GroupCubit.of(context).updateGroup(widget.group!.id, request);
      }
